/*
  ...
  1. New Columns
    - Adds `role` column to user_profiles if it doesn't exist
    - Adds `temp_access_until` column if it doesn't exist
    - Adds `payment_verified` column if it doesn't exist
    
  2. Indexes
    - Creates indexes for faster lookups
    
  3. Functions
    - Updates the handle_new_user function to properly set both id and user_id
*/
